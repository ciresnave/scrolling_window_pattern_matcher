...
fn test_repeat_and_gap_and_capture_name() {
    ...
    let patterns = vec![
        PatternBuilder::new()
            .name("gap_and_value")
            .any_elem().min_repeat(2).max_repeat(2)
            .value_elem(3).capture_name("three")
            .build(),
    ];
    ...
    let triple_twos = PatternBuilder::new()
        .name("triple_twos")
        .value_elem(2).min_repeat(3).max_repeat(3).capture_name("twos")
        .build();
    ...
    let nines_block = PatternBuilder::new()
        .name("nines_block")
        .value_elem(9).min_repeat(3).max_repeat(3).capture_name("nines")
        .any_elem().min_repeat(1).max_repeat(1)
        .value_elem(5).capture_name("five")
        .build();
...
fn test_empty_patterns_and_windows() {
    ...
    let patterns = vec![PatternBuilder::new().value_elem(1).build()];
...
fn test_single_element_patterns() {
    use scrolling_window_pattern_matcher::{PatternBuilder, ScrollingWindowPatternMatcherRef};
    let window = vec![1, 2, 3];
    let patterns = vec![PatternBuilder::new().value_elem(2).build()];
    let matcher = ScrollingWindowPatternMatcherRef::new(3);
    let named = matcher.find_matches(&window, &patterns);
    assert!(named["pattern_0"].len() > 0);
}
